...
conexion = connect(conninfo='postgresql://postgres:12345@localhost:5432/bd_flask')

# __name__ > indicar si el archivo que estamos utlizando es el archivo principal del proyecto, esto se ve en la terminal ya que si al correr el proyecto llamamos a este archivo su valor sera __main__ caso contrario obtendra otro nombre
app = Flask(__name__)

@app.route('/')
def manejar_ruta_inicio():
  return 'Bienvenido a mi  API de Flask! FLASK! Flask!'
...
def manejar_registro_usuario():
  ...
  cursor = conexion.cursor() # podemos interactuar con la base de datos (escribir y leer la informacion de ka bd)
  # %s > convierte el texto en string
...
def actualizarUsuario(id):
   ...
   if usuario:
      ...
      cursor.execute("UPDATE usuarios SET nombre = %s, apellido = %s, correo = %s WHERE id = %s", (data['nombre'], data['apellido'], data['correo'], id))
...
def devolverProductos():
   ...
   # buscar en un diccionario sobre una llave TAL VEZ exista o no es usando el metodo get
...
# python -m venv entorno_semana03
# seource entorno_semana03/Scripts/activate
